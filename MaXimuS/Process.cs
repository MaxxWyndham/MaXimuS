    ...
    public class Batch
    {
        public static void Process(string path)
        {
            foreach (var item in new DirectoryInfo(path).GetFiles("*.mxs"))
            {
                ...
                {
                    var material = mxs.MaterialList.Materials[i];
                ...
                foreach (GeomObject g in mxs.GeomObjects)
                {
                    ...
                    for (int i = 0; i < mesh.NumFaces; i++)
                    {
                        ...
                        if (hasUVs)
                        {
                            ...
                            dw.WriteLine($"f {face.X + 1 + vertoffset}/{tface.X + 1 + tvertoffset}/{n++} {face.Y + 1 + vertoffset}/{tface.Y + 1 + tvertoffset}/{n++} {face.Z + 1 + vertoffset}/{tface.Z + 1 + tvertoffset}/{n++}");
                        }
                        else
                        {
