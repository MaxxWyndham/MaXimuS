...
namespace MaXimuS.Models
{
    ...
    public class MeshFaceList : ISelfReader, ISelfWriter
    {
        ...
        public void Read(BinaryReader br, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (ascii)
                {
                    ...
                    Faces.Add(new Vector4(float.Parse(parts[3]), float.Parse(parts[5]), float.Parse(parts[7]), float.Parse(parts[Array.IndexOf(parts, "*MESH_MTLID") + 1])));
                }
                else
                {
